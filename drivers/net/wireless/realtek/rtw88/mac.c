...
static int rtw_mac_power_switch(struct rtw_dev *rtwdev, bool pwr_on)
{
	...
	if (pwr_on == cur_pwr) {
#ifdef __APPLE__
		/* macOS: The warm reboot power cycle (pwr_off_seq) powers down the PCIe interface.
		 * On x86_64 Mac, the next MMIO read will receive an Unsupported Request (UR) and
		 * cause a fatal page fault (trap 14). By returning 0 here, we skip the pwr_off_seq. */
		return 0;
#else
		return -EALREADY;
#endif
